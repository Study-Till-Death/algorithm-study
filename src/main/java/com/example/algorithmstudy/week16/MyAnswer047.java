...
public class MyAnswer047 {
    public int solution(int n, int[][] flights, int s, int e, int k) {
        ...
        pq.offer(new int[]{s, 0, -1});

        while (!pq.isEmpty()) {
            int[] current = pq.poll();
            int city = current[0];
            int cost = current[1];
            int transfers = current[2];

            if (city == e) return cost;
            if (transfers >= k) continue;

            for (int[] next : graph.get(city)) {
                int nextCity = next[0];
                int nextCost = next[1];
                pq.offer(new int[]{nextCity, cost + nextCost, transfers + 1});
            }
        }
